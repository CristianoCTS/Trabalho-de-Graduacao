...
#ifdef CONFIG_IRREMOTE_TX_HW_CARRIER
  if (_carrier >= 0) {
    irCarrierDetach(_carrier, IRpin, outputOn == LOW);
    _carrier = -1;
    return;
  }
#endif  // CONFIG_IRREMOTE_TX_HW_CARRIER
...
#ifdef CONFIG_IRREMOTE_TX_HW_CARRIER
  if (_carrier >= 0) {
    irCarrierOff(_carrier);
    return;
  }
#endif  // CONFIG_IRREMOTE_TX_HW_CARRIER
  irGpioWrite(IRpin, outputOff);
#endif
}
...
#ifdef CONFIG_IRREMOTE_TX_HW_CARRIER
  if (_carrier >= 0) {
    irCarrierOn(_carrier);
    return;
  }
#endif  // CONFIG_IRREMOTE_TX_HW_CARRIER
  irGpioWrite(IRpin, outputOn);
#endif
}
...
#if !defined(UNIT_TEST) && defined(CONFIG_IRREMOTE_TX_HW_CARRIER)
  if (_carrier >= 0) irCarrierConfig(_carrier, freq, _dutycycle);
#endif  // CONFIG_IRREMOTE_TX_HW_CARRIER
}
...
#ifndef UNIT_TEST
    // Yield to the scheduler where possible, to avoid triggering the WDT.
    irDelayMs(usec / 1000UL);  // Delay for as many whole msecs as we can.
    // Delay the remaining sub-millisecond.
    irDelayUs(usec % 1000UL);
#endif
  }
}
...
#ifndef UNIT_TEST
    irDelayUs(kMaxAccurateUsecDelay);
  irDelayUs(usec);
#endif  // UNIT_TEST
}
...
#if !defined(UNIT_TEST) && defined(CONFIG_IRREMOTE_TX_HW_CARRIER)
  if (_carrier >= 0) {
    // The LEDC peripheral emits the carrier, so we only time the mark itself.
    ledOn();
    _delayMicroseconds(usec);
    ledOff();
    const uint16_t period = onTimePeriod + offTimePeriod;
    return period ? ((usec + period - 1) / period) : 1;
  }
#endif  // CONFIG_IRREMOTE_TX_HW_CARRIER
...
int8_t IRsend::calibrate(uint16_t hz) {
  ...
  // generated.
  double actualPeriod = static_cast<double>(timeTaken) / pulses;
  // Store the difference between the actual time per period vs. calculated.
  periodOffset = (int8_t)(static_cast<double>(calcPeriod) - actualPeriod);
  return periodOffset;
}
